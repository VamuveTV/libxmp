...
int fcm_load(struct module_data *m, xmp_file f)
{
    int i, j, k;
    ...
    xmp_fread (&fh, 1, sizeof (struct fcm_header), f);
    ...
    xmp_fread (fe, 4, 1, f);	/* Skip 'SONG' pseudo chunk ID */
    ...
    xmp_fread (fe, 4, 1, f);	/* Skip 'SAMP' pseudo chunk ID */
