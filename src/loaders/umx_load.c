...
static int umx_test (xmp_file, char *, const int);
static int umx_load (struct module_data *, xmp_file, const int);
...
static int umx_test(xmp_file f, char *t, const int start)
{
	int i, offset = -1;
	uint8 buf[TEST_SIZE], *b = buf;
	uint32 id;

	if (xmp_fread(buf, 1, TEST_SIZE, f) < TEST_SIZE)
		return -1;
;
	id = readmem32b(b);
...
static int umx_load(struct module_data *m, xmp_file f, const int start)
{
	int i;
	uint8 buf[TEST_SIZE], *b = buf;
	uint32 id;
