...
static int mmd1_test(xmp_file, char *, const int);
static int mmd1_load (struct module_data *, xmp_file, const int);
...
static int mmd1_test(xmp_file f, char *t, const int start)
{
	char id[4];
	uint32 offset, len;
...
static int mmd1_load(struct module_data *m, xmp_file f, const int start)
{
	...
	for (smp_idx = i = 0; i < mod->ins; i++) {
		...
		if (expdata_offset && i < expdata.i_ext_entries) {
		    xmp_fseek(f, iinfo_offset + i * expdata.i_ext_entrsz, SEEK_SET);
