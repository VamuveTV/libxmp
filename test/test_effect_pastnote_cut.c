...
TEST(test_effect_pastnote_cut)
{
	...
	new_event(ctx, 0, 1, 0, 50, 2,  0, FX_IT_INSTFUNC, 0x00, 0, 0);
	...
	xmp_play_frame(opaque);
	fail_unless(vi->note != 59, "note still playing");
	fail_unless(vi->ins  ==  0, "not same instrument");
	fail_unless(vi->vol / 16 == 0, "didn't cut sound");
